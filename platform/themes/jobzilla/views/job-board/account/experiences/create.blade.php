...
<div class="exp-form-card">
    <div class="exp-form-header">
        <h3><i class="fa fa-briefcase me-2"></i>{{ __('Add Experience') }}</h3>
        <p>{{ __('Add your work experience details') }}</p>
    </div>
    <form action="{{ route('public.account.experiences.store') }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('company') is-invalid @enderror" id="company"
                           name="company" value="{{ old('company') }}" placeholder="{{ __('e.g. DPS School, ABC College') }}"/>
                    @error('company')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('position') is-invalid @enderror" id="position"
                           name="position" value="{{ old('position') }}" placeholder="{{ __('e.g. Mathematics Teacher, Principal') }}"/>
                    @error('position')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('employment_type') is-invalid @enderror" name="employment_type">
                        ...
                        @foreach(['full_time' => 'Full Time', 'part_time' => 'Part Time', 'contract' => 'Contract', 'internship' => 'Internship', 'freelance' => 'Freelance'] as $val => $lbl)
                            <option value="{{ $val }}" @selected(old('employment_type') == $val)>{{ $lbl }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('location') is-invalid @enderror" id="location"
                           name="location" value="{{ old('location') }}" placeholder="{{ __('e.g. New Delhi, Mumbai') }}"/>
                    @error('location')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" name="description" rows="4" placeholder="{{ __('Describe your role, responsibilities, and achievements') }}">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="exp-form-footer">
            ...
            <button type="submit" class="btn-save-exp">
                <i class="fa fa-save me-1"></i> {{ __('Save Experience') }}
            </button>
        </div>
    </form>
</div>

...
<div class="edu-form-card">
    <div class="edu-form-header">
        <h3><i class="fa fa-graduation-cap me-2"></i>{{ __('Edit Education') }}</h3>
        <p>{{ __('Update your educational qualification details') }}</p>
    </div>
    <form action="{{ route('public.account.educations.update', $education->id) }}" method="POST">
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('level') is-invalid @enderror" name="level">
                        ...
                        @foreach(['diploma' => 'Diploma', 'bachelors' => 'Bachelors', 'masters' => 'Masters', 'doctorate' => 'Doctorate'] as $val => $lbl)
                            <option value="{{ $val }}" @selected(old('level', $education->level) == $val)>{{ $lbl }}</option>
                        @endforeach
                    </select>
                    @error('level')
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('school') is-invalid @enderror" id="school"
                           name="school" value="{{ old('school', $education->school) }}" placeholder="{{ __('e.g. Delhi University') }}"/>
                    @error('school')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('specialized') is-invalid @enderror" id="specialized"
                           name="specialized" value="{{ old('specialized', $education->specialized) }}" placeholder="{{ __('e.g. Mathematics, English, Education') }}"/>
                    @error('specialized')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3 d-flex align-items-end">
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" id="is_current" name="is_current" value="1" {{ old('is_current', $education->is_current) ? 'checked' : '' }} onchange="toggleEndDate(this)">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('started_at') is-invalid @enderror" id="started_at"
                           name="started_at" value="{{ old('started_at', $education->started_at->format('Y-m-d')) }}" />
                    @error('started_at')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3" id="end-date-wrapper">
                    ...
                    <input type="date" class="form-control @error('ended_at') is-invalid @enderror" id="ended_at"
                           name="ended_at" value="{{ old('ended_at', $education->ended_at ? $education->ended_at->format('Y-m-d') : '') }}" />
                    @error('ended_at')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-12 mb-3">
                    ...
                    <textarea class="form-control @error('description') is-invalid @enderror" name="description" rows="4" placeholder="{{ __('Describe your studies, achievements, etc.') }}">{{ old('description', $education->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="edu-form-footer">
            ...
            <button type="submit" class="btn-save-edu">
                <i class="fa fa-save me-1"></i> {{ __('Update Education') }}
            </button>
        </div>
    </form>
</div>

        ...
        <div class="emp-content-card-body emp-list-wrap">
            @foreach($expiredJobs as $job)
                <a href="{{ route('public.account.jobs.edit', $job->id) }}" class="emp-list-item d-block">
                    ...
                    <div class="item-info">
                        <h6>{{ $job->name }}</h6>
                        @php $empJobDeadline = $job->effectiveApplicationDeadline(); @endphp
                        <p>{{ $job->company->name ?? '' }} &bull; Expires: {{ $empJobDeadline ? BaseHelper::formatDate($empJobDeadline) : __('Open') }}</p>
                    </div>
                    <div class="item-time">
                        <i class="ti ti-chevron-right"></i>
                    </div>

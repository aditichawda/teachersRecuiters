@php
    ...
    $jobSeekerModalProfileFields = [];
    $jobSeekerModalCompletion = 0;
    if ($account && $account->isJobSeeker()) {
        $jobSeekerModalProfileFields = [
            ['label' => __('Full Name'), 'filled' => ! empty($account->first_name)],
            ['label' => __('Mobile Number'), 'filled' => ! empty($account->phone)],
            ['label' => __('Profile Photo'), 'filled' => ! empty($account->avatar_id)],
            ['label' => __('About / Bio'), 'filled' => ! empty($account->bio)],
            ['label' => __('Resume'), 'filled' => ! empty($account->resume)],
            ['label' => __('Location/Address'), 'filled' => ! empty($account->address)],
        ];
        $filledJsModal = collect($jobSeekerModalProfileFields)->where('filled', true)->count();
        $jobSeekerModalCompletion = count($jobSeekerModalProfileFields) > 0
            ? round(($filledJsModal / count($jobSeekerModalProfileFields)) * 100)
            : 0;
    }
            ...
            $topLocations = \Botble\Location\Models\City::query()
                ...
                ->leftJoin('jb_jobs', function($join) {
                    $join->on('jb_jobs.city_id', '=', 'cities.id')
                        ...
                        ->whereRaw(\Botble\JobBoard\Models\Job::sqlNotPastEffectiveDeadline('jb_jobs'));
                })
                ->groupBy('cities.id')
                ->having('jobs_count', '>', 0)
        ...
        <div class="enl-header-right" style="display: flex; align-items: center; gap: 20px;">
            ...
            <div class="enl-header-user">
                ...
                <div class="enl-header-dropdown" id="enlUserDropdown">
                    @if($account && $account->isJobSeeker())
                        <a href="{{ route('public.account.jobseeker.dashboard') }}"><i class="fa fa-home"></i> {{ __('Dashboard') }}</a>
                        <a href="{{ route('public.account.settings') }}"><i class="fa fa-cog"></i> {{ __('Account Settings') }}</a>
                    @else
                        <a href="{{ route('public.account.dashboard') }}"><i class="fa fa-home"></i> {{ __('Dashboard') }}</a>
                        <a href="{{ route('public.account.employer.settings.edit') }}"><i class="fa fa-cog"></i> {{ __('Account Settings') }}</a>
                    @endif
                    <hr>
        ...
        <div class="enl-row">
            ...
            <div class="enl-sidebar-col" id="dashboard-sidebar">
                <div class="enl-sidebar">
                    @if($account && $account->isJobSeeker())
                        @include(Theme::getThemeNamespace('views.job-board.dashboard.layouts.partials.job-seeker-dashboard-sidebar'))
                    @else
        ...
        @if($account && $account->isJobSeeker())
            @if(\Botble\JobBoard\Facades\JobBoardHelper::isEnabledCreditsSystem())
                <div class="enl-pm-badge">
                    ...
                    <span class="enl-pm-badge-val">{{ format_credits_short($account->credits ?? 0) }}</span>
                </div>
            @endif

            <div class="enl-pm-comp">
                <h6 class="enl-pm-comp-title">{{ __('Profile Completion') }}</h6>
                <div class="enl-pm-bar">
                    <div class="enl-pm-bar-fill" style="width: {{ $jobSeekerModalCompletion }}%"></div>
                </div>
                <span class="enl-pm-comp-text">{{ $jobSeekerModalCompletion }}% {{ __('Complete') }}</span>
            </div>

            <div>
                @foreach($jobSeekerModalProfileFields as $pf)
            ...
            @if($jobSeekerModalCompletion < 100)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Sidebar navigation:
    // Disable AJAX swap only for pages where DataTables/UI scripts
    // break on partial HTML replacement (Institution/Applicants/Wallet/Invoices/Jobs).
    // For all other account pages, keep AJAX navigation for smoother UX.
    const AJAX_SWAP_DISABLED_PATHS = [
        '/account/jobs',
        '/account/applicants',
        '/account/companies',
        '/account/wallet',
    ...
    document.addEventListener('click', function(e) {
        ...
        if (href.includes('/account/')) {
            // For problematic pages, do full navigation (no AJAX swap)
            var shouldDisableAjaxSwap = AJAX_SWAP_DISABLED_PATHS.some(function(p) {
                return href.includes(p);
            });

            if (shouldDisableAjaxSwap) {
                return;
            }
            ...
            .then(function(html) {
                ...
                function reinitDataTablesInMain() {
                    ...
                    // Support both:
                    // 1) core/table tables (usually identified by `id` only)
                    // 2) shortcode tables (may use `data-table`)
                    $main.find('table').each(function() {
                        var $table = $(this);
                        var tableId = $table.attr('id');
                        if (!tableId) return;

                        // If already initialized, just reload data
                        ...
                        // If LaravelDataTables already exists for this table, try a draw.
                        if (window.LaravelDataTables && window.LaravelDataTables[tableId]) {
                            try {
                                window.LaravelDataTables[tableId].draw(false);
                                return;
                            } catch (e) {
                                console.warn('[DataTables] Draw failed for', tableId, e);
                            }
                        }

                        // Best-effort: re-exec init scripts if present
                ...
                setTimeout(function() {
                    ...
                    scriptsToExecute.forEach(function(scriptData) {
                        ...
                        // Do NOT skip inline scripts by `id`.
                        // DataTables initialization scripts can reuse ids across pages; skipping them
                        // prevents tables like Invoices from fetching/rendering rows.

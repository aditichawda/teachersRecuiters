...
<form id="jobPostForm" method="POST" action="{{ $formAction }}" enctype="multipart/form-data">
    ...
    <div class="jp-card jp-card-collapsible" data-section="1">
        ...
        <div class="jp-group">
            ...
            <textarea name="content" id="job_description" class="jp-textarea" rows="6" placeholder="Enter detailed job description or use AI to generate..." required>{{ old('content', isset($job) ? strip_tags($job->content) : '') }}</textarea>            <!-- <textarea name="content" id="job_description" class="jp-textarea" rows="6" placeholder="Enter detailed job description or use AI to generate..." required>{{ old('content', optional($job)->content ?? '') }}</textarea> -->

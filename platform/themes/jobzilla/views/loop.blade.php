                    ...
                    @if($index === 0 && $posts->currentPage() === 1)
                        ...
                        <article class="blog-card blog-featured">
                            <div class="blog-card-img">
                                ...
                                <span class="blog-card-date">{{ Theme::formatDate($post->created_at) }}</span>
                                @if($post->categories->count())
                                    <span class="blog-card-category">{{ $post->categories->first()->name }}</span>
                                @endif
                            </div>
                            <div class="blog-card-body">
                                <span class="featured-label">⭐ Featured</span>
                                <div class="blog-card-meta">
                                    <span class="author-avatar">{{ strtoupper(substr($post->author->name ?? 'A', 0, 1)) }}</span>
                                    <span class="author-name">{{ $post->author->name ?? 'Admin' }}</span>
                                </div>
                                <h2 class="blog-card-title">
                                    <a href="{{ $post->url }}">{!! BaseHelper::clean($post->name) !!}</a>
                                </h2>
                                <p class="blog-card-excerpt">{!! Str::limit(BaseHelper::clean($post->description), 200) !!}</p>
                    ...
                    @else
                        ...
                        <article class="blog-card">
                            ...
                            <div class="blog-card-body">
                                ...
                                <p class="blog-card-excerpt">{!! Str::limit(BaseHelper::clean($post->description), 120) !!}</p>
                                <a href="{{ $post->url }}" class="blog-card-link">
                                    Read More
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>
                                </a>

    ...
    {!! Form::open(['url' => JobBoardHelper::getJobsPageURL(), 'method' => 'GET']) !!}
        <div class="row">
            <div class="form-group col-xl-4 col-lg-5 col-md-6">
                ...
                <select class="wt-search-bar-select selectpicker-keyword" name="keyword" id="home_keyword_search">
                    <option value="">{{ __('Select job role...') }}</option>
                    @php
                        $selectedKeyword = request('keyword', '');
                        $homeJobRoles = collect();
                        try {
                            if (\Illuminate\Support\Facades\Schema::hasTable('jb_roles')) {
                                $q = \Illuminate\Support\Facades\DB::table('jb_roles')->select(['id', 'name']);
                                if (\Illuminate\Support\Facades\Schema::hasColumn('jb_roles', 'status')) {
                                    $q->where(function ($s) {
                                        $s->where('status', \Botble\Base\Enums\BaseStatusEnum::PUBLISHED)
                                            ->orWhere('status', 1)
                                            ->orWhere('status', '1')
                                            ->orWhereNull('status');
                                    });
                                } elseif (\Illuminate\Support\Facades\Schema::hasColumn('jb_roles', 'is_active')) {
                                    $q->where('is_active', 1);
                                }
                                $homeJobRoles = $q->orderBy('name')->limit(200)->get();
                            }
                        } catch (\Throwable $e) {
                            $homeJobRoles = collect();
                        }
                    @endphp
                    @foreach ($homeJobRoles as $r)
                        @php($roleName = $r->name ?? '')
                        @if (is_string($roleName) && trim($roleName) !== '')
                            <option value="{{ $roleName }}" {{ $selectedKeyword == $roleName ? 'selected' : '' }}>
                                {{ $roleName }}
                            </option>
                        @endif
                    @endforeach
            ...
            @if (is_plugin_active('location'))
                <div class="form-group col-xl-4 col-lg-5 col-md-6">
                    ...
                    <div class="home-city-search-wrapper" style="position: relative;">
                        <input type="text" name="state_search" id="home_state_search" class="wt-search-bar-select home-city-search-input" placeholder="{{ __('Select State') }}" autocomplete="off" value="{{ request('state_search', '') }}" />
                        <input type="hidden" name="state_id" id="home_state_id" value="{{ request('state_id', '') }}" />
...
<script>
    // Keyword select (Job roles) - open dropdown on click, type optional
    (function() {
                    ...
                    $keywordSelect.on('select2:open', function() {
                        ...
                        const setPlaceholder = function(attempts) {
                            ...
                            if ($searchInput.length) {
                                $searchInput.attr('placeholder', '{{ __('Type to search...') }}');
                            } else {
                                setTimeout(function() { setPlaceholder(attempts + 1); }, 50);
                            }
                        };
                        setPlaceholder();
                    });
        ...
        function initStateAutocomplete() {
            ...
            function render(items) {
                ...
                if (!items || !items.length) {
                    box.innerHTML = '<div class="home-city-no-results">{{ __('No results found') }}</div>';
            ...
            function search() {
                ...
                box.innerHTML = '<div class="home-city-loading">{{ __('Searching...') }}</div>';
                box.style.display = 'block';
                t = setTimeout(function () {
                    ajaxGet((window.siteUrl || window.location.origin) + '/ajax/search-states', { k })
                        .then((res) => render((res && res.data) ? res.data : []))
                        .catch(() => {
                            box.innerHTML = '<div class="home-city-no-results">{{ __('Error searching') }}</div>';
                            box.style.display = 'block';
                        });
                }, 250);
            }

            ...
            @foreach($group['options'] as $value => $label)
                <li>
                    <div class="form-check">
                        <input type="checkbox" class="form-check-input" name="institution_type[]" id="company-institution-type-{{ md5($value) }}" value="{{ $value }}" @checked(in_array($value, (array) request()->query('institution_type', [])))>
                        <label class="form-check-label" for="company-institution-type-{{ md5($value) }}">{{ $label }}</label>
                    </div>
                </li>
            @endforeach
        @endforeach

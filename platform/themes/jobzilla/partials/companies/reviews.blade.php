...
<div class="mt-4 pt-5 position-relative review-listing">
    <h4 class="fs-17 fw-semibold mb-3">{{ __(":company's :review", ['company' => $company->name, 'review' => $reviewsCount > 1 ? __('Reviews') : __('Review')]) }}</h4>
    ...
    <form action="{{ route('public.reviews.create') }}" method="post" class="company-review-form mt-4 pt-3">
        ...
        <h5 class="fs-17 fw-semibold mb-2">{{ __('Reviews for :company', ['company' => $company->name]) }}</h5>
        @guest('account')
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="review" name="review" style="height: auto" @disabled(! $canReviewCompany) rows="5" placeholder="{{ __('Add your review') }}"></textarea>
                </div>
            </div>
        </div>
        <div class="text-end">
            <button type="submit" class="site-button" @disabled(! $canReviewCompany)>
                {{ __('Submit Review') }}
            </button>
        </div>
    </form>

...
(function ($) {
  ...
  var $companiesList = $companiesContainer.find('.companies-content');
  var $companiesLoader = $companiesContainer.find('#companies-loader-overlay');

  // Create loader if it doesn't exist
  if (!$companiesLoader.length && $companiesList.length) {
    var loaderHtml = '<div class="blue-loader-overlay" id="companies-loader-overlay" style="display: none !important; z-index: 100 !important;"><div class="blue-loader-wrapper"><div class="blue-loader large"></div><p class="blue-loader-text">Loading companies...</p></div></div>';
  ...
  function showCompaniesLoader() {
    if ($companiesLoader.length) {
      ...
      // Create loader if it doesn't exist
      var _loaderHtml = '<div class="blue-loader-overlay" id="companies-loader-overlay" style="display: flex !important; visibility: visible !important; opacity: 1 !important; z-index: 100 !important;"><div class="blue-loader-wrapper"><div class="blue-loader large"></div><p class="blue-loader-text">Loading companies...</p></div></div>';
      $companiesList.before(_loaderHtml);
      $companiesLoader = $('#companies-loader-overlay');
    }
  }
    ...
    filterAjax = $.ajax({
      ...
      success: function success(response) {
        ...
        if (!$companiesList.find('#companies-loader-overlay').length) {
          var _loaderHtml2 = '<div class="blue-loader-overlay" id="companies-loader-overlay" style="display: none !important; z-index: 100 !important;"><div class="blue-loader-wrapper"><div class="blue-loader large"></div><p class="blue-loader-text">Loading companies...</p></div></div>';
          $companiesList.before(_loaderHtml2);
        }
        $companiesLoader = $('#companies-loader-overlay');
  ...
  companies.on('submit', '#companies-filter-form', function (e) {
    ...
    filterForm.find('select, input[type="text"], input[type="hidden"]').each(function () {
      ...
      if (name && value && !$(this).is(':checkbox')) {
        ...
        if (name === 'city_id') {
          var existing = form.find("input[name=\"city_id\"]");
          if (existing.length) {
            existing.val(value);
          } else {
            form.append("<input type=\"hidden\" name=\"city_id\" value=\"".concat(value, "\">"));
          }
        } else if (name === 'city_search') {
          // Don't add city_search to form, only city_id
        } else {
          var _existing = form.find("input[name=\"".concat(name, "\"]"));
          if (_existing.length) {
            _existing.val(value);
          } else {
            form.append("<input type=\"hidden\" name=\"".concat(name, "\" value=\"").concat(value, "\">"));
  ...
  companies.on('change', '.location-filter[data-target="state"]', function () {
    ...
    var stateSelect = $('.location-filter[data-target="city"]').closest('.form-group').prev().find('.location-filter[data-target="state"]');
    var citySelect = $('.location-filter[data-target="city"]');
    if (countryId) {
      $.ajax({
        ...
        success: function success(states) {
          ...
          if (states && states.length) {
            ...
            states.forEach(function (state) {
              stateSelect.append("<option value=\"".concat(state.id, "\">").concat(state.name, "</option>"));
            });
        ...
        success: function success(cities) {
          ...
          if (cities && cities.length) {
            ...
            cities.forEach(function (city) {
              citySelect.append("<option value=\"".concat(city.id, "\">").concat(city.name, "</option>"));
            });
            citySelect.selectpicker('refresh');
          }
        }
  ...
  function initCompanyCitySearch() {
    var $cityInput = $('#company_city_search');
    var $cityId = $('#company_city_id');
    var $suggestions = $('#company-city-suggestions');
        ...
        $.ajax({
          ...
          success: function success(response) {
            ...
            cities.forEach(function (city) {
              ...
              html += '<div class="company-city-suggestion-item" ' + 'data-id="' + city.id + '" ' + 'data-name="' + city.name + '">' + '<div class="city-name">' + city.name + '</div>' + (locationParts.length ? '<div class="city-location">' + locationParts.join(', ') + '</div>' : '') + '</div>';
            });
    ...
    $cityInput.on('keydown', function (e) {
      var $items = $suggestions.find('.company-city-suggestion-item');

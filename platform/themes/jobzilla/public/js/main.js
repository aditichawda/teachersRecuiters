...
(function ($) {
  ...
  function home_city_search() {
    ...
    var $cityId = $('#home_city_id');
    var $suggestions = $('#home-city-suggestions');
        ...
        $.ajax({
          ...
          success: function success(response) {
            ...
            cities.forEach(function (city) {
              ...
              html += '<div class="home-city-suggestion-item" ' + 'data-id="' + city.id + '" ' + 'data-name="' + city.name + '">' + '<div class="city-name">' + city.name + '</div>' + (locationParts.length ? '<div class="city-location">' + locationParts.join(', ') + '</div>' : '') + '</div>';
            });
    ...
    $cityInput.on('keydown', function (e) {
      var $items = $suggestions.find('.home-city-suggestion-item');
  ...
  function home_category_search() {
    var $categoryInput = $('#home_category_search');
    var $categoryId = $('#home_keyword_id');
    var $suggestions = $('#home-category-suggestions');
    var searchTimeout = null;
    var activeSuggestionIndex = -1;
    ...
    function showAllCategories() {
      ...
      if (!window.jobCategories || window.jobCategories.length === 0) {
        ...
        $suggestions.html('<div class="home-category-no-results">No job roles available</div>').addClass('show').css({
      ...
      window.jobCategories.forEach(function (category) {
        html += '<div class="home-category-suggestion-item" ' + 'data-id="' + category.id + '" ' + 'data-name="' + category.name + '">' + '<div class="city-name">' + category.name + '</div>' + '</div>';
      });
      ...
      $suggestions.html(html).addClass('show').removeAttr('style').attr('style', 'display: block !important; visibility: visible !important; opacity: 1 !important; position: absolute !important; top: 100% !important; left: 0 !important; right: 0 !important; z-index: 10000 !important;').show();
    ...
    var $dropdownArrow = $('#home-category-dropdown-arrow');
    ...
    function initializeSearch() {
      ...
      function showCategories(keyword) {
        ...
        $suggestions.html(html).addClass('show').attr('style', 'display: block !important; visibility: visible !important; opacity: 1 !important; position: absolute !important; top: 100% !important; left: 0 !important; right: 0 !important; z-index: 10000 !important;').show();
      ...
      // Keyboard navigation for suggestions
      $categoryInput.on('keydown', function (e) {
        var $items = $suggestions.find('.home-category-suggestion-item');
        if (!$suggestions.is(':visible') || $items.length === 0) return;
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          activeSuggestionIndex = Math.min(activeSuggestionIndex + 1, $items.length - 1);
      ...
      var existingKeyword = $categoryId.val();
      if (existingKeyword && window.jobCategories) {
        var role = window.jobCategories.find(function (cat) {
  ...
  function job_type_filter() {
    ...
    $('.btn-filter-wrap').on('click', '.btn-filter', function (e) {
      ...
      owl.owlFilter(filter_data, function (_owl) {
        ...
        var $carousel = $('.owl-carousel-filter');
        var $sectionContent = $carousel.closest('.section-content');
        var $noJobsMessage = $sectionContent.find('.no-jobs-message');
        ...
        setTimeout(function () {
          ...
          if (filter_data === '*') {
            ...
            var filterClass = filter_data.replace('.', '');
  ...
  $(document).ready(function () {
    ...
    var isJobsPage = window.location.pathname.includes('/jobs') || $('#jobs-top-filter-form').length > 0 || $('#home_category_search').closest('.jobs-top-filter').length > 0;
    if (!isJobsPage) {

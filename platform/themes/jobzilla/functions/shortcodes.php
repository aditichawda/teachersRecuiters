    ...
    if (is_plugin_active('job-board')) {
        ...
        add_shortcode('jobs-by-categories', __('Jobs by Categories'), __('Jobs by Categories'), function ($shortcode) {
            ...
            if (is_plugin_active('location')) {
                ...
                // Prefer showing cities that actually have active jobs
                try {
                    if (is_plugin_active('job-board')) {
                        $locations = City::query()
                            ...
                            ->leftJoin('jb_jobs', function ($join): void {
            ...
            // Job roles (jb_roles table)
            $jobRoleLimit = (int) ($shortcode->job_role_limit ?: 20);
            $jobRoles = collect();
            try {
                if (\Illuminate\Support\Facades\Schema::hasTable('jb_roles')) {
                    $jobRoles = DB::table('jb_roles')
                        ->select(['id', 'name'])
                        ->where(function ($q): void {
                            // support either "status" or "is_active" flags if present
                        ...
                        ->map(function ($r) {
                            $name = $r->name ?? null;
                            $jobsUrl = \Botble\JobBoard\Facades\JobBoardHelper::getJobsPageURL();
                            $url = $jobsUrl ? $jobsUrl . '?' . http_build_query(['keyword' => $name]) : '#';
            ...
            // Institution types (same list as registration)
            ...
            try {
                ...
                $flat = [];
                foreach ((array) $institutionTypeGroups as $group) {
                ...
                $jobsUrl = \Botble\JobBoard\Facades\JobBoardHelper::getJobsPageURL();
                $institutionTypes = collect($flat)
                    ->take($institutionTypeLimit)
                    ->map(function (array $item) use ($jobsUrl) {
                        $value = $item['value'];
                        $url = $jobsUrl
                            ? $jobsUrl . '?' . http_build_query(['institution_type' => [$value]])
                            : '#';

...
class Theme implements ThemeContract
{
    ...
    public function getLogoImage(
        ...
        $width = theme_option('logo_width') ?: $maxWidth;

        $existingStyle = trim((string) ($attributes['style'] ?? ''));
        $styles = [];
        ...
        if ($height) {
            ...
            $styles[] = sprintf($maxHeightStyle, is_numeric($height) ? "{$height}px" : $height);
        }

        if ($width) {
            $maxWidthStyle = 'max-width: %s';

            if (setting('optimize_inline_css', 0)) {
                $maxWidthStyle = 'max-width: %s !important';
            }

            $styles[] = sprintf($maxWidthStyle, is_numeric($width) ? "{$width}px" : $width);
        }

        if ($existingStyle !== '') {
            array_unshift($styles, $existingStyle);
        }

        if (! empty($styles)) {
            $styles = array_filter(array_map(static function ($style) {
                return trim(trim((string) $style), ';');
            }, $styles));

        ...
        try {
            ...
            if (! $keyword || strlen($keyword) < 2) {
                if ($defaultCountry === '1' || $defaultCountry === 'true') {
                    ...
                    if ($countryForDefault) {
                        $query->where(function ($q) use ($countryForDefault) {
                            $q->where('cities.country_id', $countryForDefault->id)
                                ->orWhereHas('state', fn ($s) => $s->where('country_id', $countryForDefault->id));
                        });
                        $total = $query->count();

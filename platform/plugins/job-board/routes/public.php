...
Route::group(['namespace' => 'Botble\JobBoard\Http\Controllers\Fronts', 'middleware' => ['web', 'core']], function (): void {
    // New URL for companies listing page (keep content/logic same, only URL changes)
    Route::get('institutes', function () {
        \Botble\SeoHelper\Facades\SeoHelper::setTitle(trans('plugins/job-board::messages.companies'));

        Theme::breadcrumb()
            ->add(__('Home'), url('/'))
            ->add(trans('plugins/job-board::messages.companies'), url('/institutes'));

        return Theme::scope('institutes')->render();
    })->name('public.institutes');

    // Backward-compatible redirects (old "companies" URLs)
    Route::redirect('companies', 'institutes', 301);
    Route::get('companies/{slug}', function (string $slug) {
        return redirect('institutes/' . ltrim($slug, '/'), 301);
    })->where('slug', '.*');
    ...
    Theme::registerRoutes(function (): void {
        ...
        Route::get('ajax/search-job-titles', [
            'as' => 'public.ajax.search-job-titles',
            'uses' => 'PublicController@ajaxSearchJobTitles',
        ]);
        ...
        Route::get(SlugHelper::getPrefix(Company::class, 'institutes') . '/{slug}', [
            'as' => 'public.company',

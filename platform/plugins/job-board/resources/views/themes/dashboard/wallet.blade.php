...
@section('content')
                                ...
                                <li class="d-flex justify-content-between align-items-center py-1 border-bottom">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        @if($account->isEmployer() && is_array($item) && !empty($item['credits']))
                                            ...
                                            @elseif($key === \Botble\JobBoard\Models\CreditConsumption::FEATURE_ADDITIONAL_EMPLOYER_PROFILE)
                                                @php $companiesCount = count($companies ?? []); @endphp
                                                @if($companiesCount < 1)
                                                    <button type="button" class="btn btn-xs btn-outline-secondary" disabled title="{{ trans('plugins/job-board::messages.wallet_additional_profile_locked_hint') }}">
                                                        {{ __('Use credits') }}
                                                    </button>
                                                    <a href="{{ route('public.account.companies.create') }}" class="btn btn-xs btn-outline-primary" title="{{ trans('plugins/job-board::messages.wallet_create_first_institution') }}">
                                                        {{ trans('plugins/job-board::messages.wallet_create_first_institution') }}
                                                    </a>
                                                @else
                                                    <button
                                                        type="button"
                                                        ...
                                                        title="{{ __('Deduct coins to add another institution.') }}"
                                                    >
                                                        {{ __('Use credits') }}
                                                    </button>
                                                @endif
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var activeCredits = 0;
        var activeLabel = '';
        var companiesCreateUrl = '{{ route('public.account.companies.create') }}' + '?credits_already_deducted=1';
        ...
        if (featureConfirmBtn) {
            featureConfirmBtn.addEventListener('click', function() {
                ...
                // Special flow for Additional Employer Profile:
                // Deduct credits first, then redirect to companies/create with credits_already_deducted=1
                if (activeFeatureKey === '{{ \Botble\JobBoard\Models\CreditConsumption::FEATURE_ADDITIONAL_EMPLOYER_PROFILE }}') {
                    fetch('{{ route('additional-employer-profile.deduct') }}', {
                        method: 'POST',
                        headers: {
                    ...
                    .then(function(data) {
                        ...
                        if (ok) {
                            window.location.href = companiesCreateUrl;
                        } else {

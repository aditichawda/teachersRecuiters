            ...
            <div class="col">
                <div @class(['card card-md box-package h-100', 'active' => $package->is_default])>
                    ...
                    <div class="card-body">
                        ...
                        @endphp
                        @if($basePrice > 0)
                            <div class="box-package-price d-flex align-items-end flex-wrap gap-2">
                                <h4 class="mb-0">{{ format_price($totalWithGst, $currency) }}</h4>
                                <span class="text-muted">({{ __('incl. GST') }})</span>
                                <span class="text-muted">/{{ $package->number_of_listings }} {{ trans('plugins/job-board::dashboard.posts_count') }}</span>
                                <div class="w-100 small text-muted mt-1">
                                    {{ __('Base') }}: {{ format_price($basePrice, $currency) }} · {{ __('GST (18%)') }}: {{ format_price($gstAmount, $currency) }}
                                </div>
                            </div>
                        @else

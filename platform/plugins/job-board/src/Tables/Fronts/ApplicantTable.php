    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('job_id', function (JobApplication $item) {
                ...
                return '<a href="' . e($jobUrl) . '" target="_blank" class="text-primary text-decoration-none">' . e($job->name) . ' ' . BaseHelper::renderIcon('ti ti-external-link') . '</a>';
            })
                ...
                ->getValueUsing(function (FormattedColumn $column) {
                    try {
                        ...
                        if ($account && $account->getKey() && $account->is_public_profile && ! empty($account->url)) {
                            $candidateUrl = (string) $account->url;

                            return '<a href="' . e($candidateUrl) . '" target="_blank" title="' . e($candidateUrl) . '" class="text-primary text-decoration-none">' . e($account->name) . ' ' . BaseHelper::renderIcon('ti ti-external-link') . '</a>';
                        }

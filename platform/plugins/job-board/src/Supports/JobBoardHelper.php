...
class JobBoardHelper
{
    ...
    public function getJobFilters(array|Request $inputs): array
    {
        ...
        $params = [
            ...
            'company_id' => (int) Arr::get($inputs, 'company_id'),
        ...
        $query = JobType::query()
            ->select([
                ...
                DB::raw('(SELECT COUNT(DISTINCT jb_jobs.id)
                    ...
                    AND ' . Job::sqlNotPastEffectiveDeadline('jb_jobs') . '
        ...
        $query = JobExperience::query()
            ...
            ->leftJoin('jb_jobs', function ($join) use ($data): void {
                $join->on('jb_job_experiences.id', '=', 'jb_jobs.job_experience_id')
                    ...
                    ->whereRaw(Job::sqlNotPastEffectiveDeadline('jb_jobs'));
    ...
    /**
     * Job description / full content for public detail: CKEditor HTML as-is, or plain text
     * (AI-generated) with bullets, numbered lines, and paragraphs formatted.
     */
    public function formatJobRichContent(?string $content): string
    {
        ...
        if ($this->jobBodyLooksLikeHtml($content)) {
            $cleaned = (string) BaseHelper::clean($content);
            // Paste/Word HTML that is only inline tags (e.g. <span>) still renders as one line — reformat as plain text.
            if ($this->htmlJobBodyLacksBlockStructure($cleaned)) {
                $plain = html_entity_decode(strip_tags($cleaned), ENT_QUOTES | ENT_HTML5, 'UTF-8');

                return (string) BaseHelper::clean($this->plainTextJobBodyToHtml($plain));
            }

            return $cleaned;
        }

        return (string) BaseHelper::clean($this->plainTextJobBodyToHtml($content));
    }

    protected function jobBodyLooksLikeHtml(string $content): bool
    {
        return (bool) preg_match(
            '/<\s*(p|div|ul|ol|li|br\s*\/?|h[1-6]|table|span|strong|em|b|i|blockquote)\b/i',
            $content
        );
    }

    /**
     * True when "HTML" has no real block/line-break structure (single blob of inline markup).
     */
    protected function htmlJobBodyLacksBlockStructure(string $html): bool
    {
        return ! preg_match('/<(p|div|ul|ol|li|br\s*\/?|h[1-6]|table|blockquote)\b/i', $html);
    }

    /**
     * Split one physical line that contains multiple "• item" segments into multiple lines
     * so plainTextJobBodyToHtml can build proper &lt;ul&gt; lists.
     *
     * @return list<string>
     */
    protected function expandInlineBulletSegments(string $line): array
    {
        $line = rtrim($line);
        if ($line === '') {
            return [''];
        ...
        $t = trim($line);
        // Already a line-start bullet / numbered line — leave as-is
        if (preg_match('/^[\-\*•]\s+/u', $t) || preg_match('/^\d+[\.\)]\s+/', $t)) {
            return [$line];
        }
    ...
    /**
     * Convert plain-text job body to simple HTML: paragraphs, ul/ol, optional section labels.
     */
    protected function plainTextJobBodyToHtml(string $text): string
    {
        $text = str_replace(["\r\n", "\r"], "\n", $text);
        $rawLines = explode("\n", $text);
        $lines = [];
        foreach ($rawLines as $line) {
            foreach ($this->expandInlineBulletSegments($line) as $expanded) {
                $lines[] = $expanded;
            }
        }
        ...
        $flushPara = function () use (&$html, &$para): void {
            if ($para === []) {
                return;
            }
        ...
        $flushList = function () use (&$html, &$list, &$listTag): void {
            if ($list === [] || $listTag === null) {
                $list = [];
                $listTag = null;
        ...
        foreach ($lines as $line) {
            ...
            // Single-line section label (e.g. "Key Responsibilities:")
            if (preg_match('/^[^:]+:\s*$/u', $t) && mb_strlen($t) < 200) {
                $flushPara();
                $html[] = '<p><strong>' . e($t) . '</strong></p>';
    ...
    /**
     * Label for job hiring_institution_type (values from institution-type-groups.php).
     */
    public function institutionTypeLabel(?string $value): ?string
    {
        if ($value === null || $value === '') {
            return null;
        }

        $path = base_path('platform/plugins/job-board/resources/data/institution-type-groups.php');
        if (is_file($path)) {
            $groups = require $path;
    ...
    /**
     * One line for job detail: "Application accepted from which location".
     */
    public function applicationAcceptedLocationLine(Job $job): ?string
    {
        $type = $job->application_location_type;
        $rawLocs = $job->application_locations;
        $hasLocations = $rawLocs !== null && $rawLocs !== '' && $rawLocs !== '[]';

        if (! $type && ! $hasLocations) {
            return null;
        }

        $type = $type ?: 'nearby';

        if (in_array($type, ['anywhere', 'across_india'], true)) {
            return __('India');
        }

        if ($type === 'nearby') {
            return __('Nearby areas only');
        }

        if ($type === 'specific') {
            if (is_array($rawLocs)) {
                $locs = $rawLocs;
            } elseif (is_string($rawLocs) && $rawLocs !== '') {
                $decoded = json_decode($rawLocs, true);
                $locs = is_array($decoded)
                    ? $decoded
                    : preg_split('/\s*,\s*/', $rawLocs, -1, PREG_SPLIT_NO_EMPTY);
            } else {
                $locs = [];
            }

            $parts = [];
            foreach ((array) $locs as $item) {
                if (is_string($item)) {
                    $t = trim($item);
                    if ($t !== '') {
                        $parts[] = $t;
                ...
                if (is_array($item)) {
                    foreach (['name', 'label', 'city', 'location'] as $key) {

...
class JobSeekerPackageContext
{
    ...
    public static function forAccount(Account $account): self
    {
        ...
        if (! $package) {
            ...
            /**
             * Defensive fallback: if default job-seeker plan is missing (migration not run),
             * create a free "Basic Profile Plan" so job seekers can still apply to jobs.
             * NOTE: Do not hardcode apply limit here; always use package.job_apply_limit.
             */
            if (! $package && Schema::hasTable('jb_packages') && Schema::hasTable('jb_currencies')) {
                try {
                    $currency = Currency::query()->where('is_default', 1)->first()
                        ?? Currency::query()->orderBy('order')->first();

                    $currencyId = $currency ? (int) $currency->getKey() : 1;
                    ...
                    $package = Package::query()->firstOrCreate(
                        ...
                        [
                            ...
                            'worth' => null,
                            'description' => 'Free plan: Basic CV, unlimited validity.',
                    ...
                    if ($package) {
                        // Ensure it is default + published even if existed with wrong flags.
                        // Keep job_apply_limit untouched; it must come from package configuration.
                        $package->update([
                            'is_default' => 1,
                            'status' => 'published',
                        ]);
                    ...
                    // If fallback creation fails, continue without package (apply will be blocked)
                }
            }
        }
        ...
        // Default/free: period should be stable (do NOT use "now", otherwise used count never increments).
        // Otherwise use periodStart from pivot/purchase above, or account age.
        if ($package && $periodStart === null) {
            $periodStart = $account->created_at ?: Carbon::now();
        }

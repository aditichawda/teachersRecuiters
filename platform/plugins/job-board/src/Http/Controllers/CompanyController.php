    ...
    public function generateSlugs(Request $request)
    {
        if (! SlugHelper::isSupportedModel(Company::class)) {
            return $this
                ...
                ->setPreviousUrl(route('companies.index'))
                ->setError()
                ->setMessage(__('Slug generation not supported for this model.'));
        }
        ...
        Company::query()
            ->select(['id', 'name'])
            ->orderBy('id')
            ->chunkById(200, function ($companies) use (&$created, &$updated, &$failed, $prefix): void {
                foreach ($companies as $company) {
                    try {
                        $existing = SlugHelper::getSlug(null, $prefix, Company::class, $company->getKey());
                        ...
                        \Log::warning('JobBoard: Failed to generate slug for company ' . $company->id, [
                            'error' => $e->getMessage(),
                        ]);
                    }
                }
        ...
        $message = sprintf(
            'Company slugs generated successfully. Created: %d, Updated: %d, Failed: %d.',
            $created,
            $updated,
            $failed
        );

...
class JobController extends BaseController
{
    ...
    public function generateSlugs(Request $request)
    {
        ...
        Job::query()
            ->select(['id', 'name'])
            ->orderBy('id')
            ->chunkById(200, function ($jobs) use (&$created, &$updated, &$failed, $prefix): void {
                foreach ($jobs as $job) {
                    try {
                        $existing = SlugHelper::getSlug(null, $prefix, Job::class, $job->getKey());
                        ...
                        $failed++;
                        \Log::warning('JobBoard: Failed to generate slug for job ' . $job->id, [
                            'error' => $e->getMessage(),
                        ]);
                    }
                }
        ...
        $message = sprintf(
            'Job slugs generated successfully. Created: %d, Updated: %d, Failed: %d.',
            $created,
            $updated,
            $failed
        );

    ...
    public function index()
    {
        ...
        $expiredJobs = Job::query()
            ...
            ->where(function ($query): void {
                ...
                $now = Carbon::now();
                $until = Carbon::now()->addDays($warningDays);

                $query->where(function ($q) use ($now, $until): void {
                    $q->whereNotNull('jb_jobs.application_closing_date')
                        ->where('jb_jobs.application_closing_date', '>=', $now)
                        ->where('jb_jobs.application_closing_date', '<=', $until);
                })->orWhere(function ($q) use ($now, $until): void {
                    $q->whereNull('jb_jobs.application_closing_date')
                        ->where('jb_jobs.never_expired', false)
                        ->whereNotNull('jb_jobs.expire_date')
                        ->where('jb_jobs.expire_date', '>=', $now)
                        ->where('jb_jobs.expire_date', '<=', $until);
                });
            })
            ->with('company')
            ->withCount(['applicants'])
            ->orderByRaw('COALESCE(jb_jobs.application_closing_date, jb_jobs.expire_date) ASC')
            ->get();

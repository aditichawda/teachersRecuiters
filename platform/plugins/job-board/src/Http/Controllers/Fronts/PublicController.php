...
class PublicController extends BaseController
{
    ...
    public function ajaxSearchJobTitles(Request $request)
    {
        $request->headers->set('Accept', 'application/json');

        $keyword = BaseHelper::stringify($request->query('k') ?: $request->query('keyword'));

        // Preferred: jb_roles (Job roles) as requested
        $titles = [];

        if (Schema::hasTable('jb_roles')) {
            $q = DB::table('jb_roles')->select(['id', 'name']);

            if (Schema::hasColumn('jb_roles', 'status')) {
                // status may be enum string ('published') or numeric (1) depending on DB
                $q->where(function ($s): void {
                    $s->where('status', BaseStatusEnum::PUBLISHED)
                        ->orWhere('status', 1)
                        ->orWhere('status', '1')
                        ->orWhereNull('status');
                });
            } elseif (Schema::hasColumn('jb_roles', 'is_active')) {
                $q->where('is_active', 1);
            }
            ...
            $roles = $q->orderBy('name')->limit(20)->get();

            $titles = collect($roles)
                ->pluck('name')
            ...
            // If table exists but status flags don't match, retry without status filter
            if (! $titles && Schema::hasColumn('jb_roles', 'name')) {
                $q2 = DB::table('jb_roles')->select(['id', 'name']);
                ...
                $roles2 = $q2->orderBy('name')->limit(20)->get();

                $titles = collect($roles2)
                    ->pluck('name')
                    ->filter(fn ($t) => is_string($t) && trim($t) !== '')
                    ->unique()
                    ->values()
        ...
        // Fallback (if jb_roles empty/missing): job names from jobs table
        if (! $titles) {
        ...
        return $this->httpResponse()->setData($titles);
    }
        ...
        try {
            ...
            if (! empty($job->author_id)) {
                try {
                    ...
                    if ($job->author instanceof \Botble\JobBoard\Models\Account) {
                        $employerAccount = $job->author;
                    } elseif ($job->company && $job->company->exists) {
                        $employerAccount = $job->company->accounts()->first();
                    }

                    if (! $employerAccount) {
                        \Log::warning('[JOB_APPLICATION] Employer account not found for notification', [
                            'job_id' => $job->id,

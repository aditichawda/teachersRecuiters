...
class LoginController extends Controller
{
    ...
    public function showLoginForm()
    {
        ...
        Theme::breadcrumb()->add(trans('plugins/job-board::messages.login'), route('public.account.login'));
        ...
        if ($this->attemptLogin($request)) {
            ...
            if ($account->email_verified_at === null) {
                ...
                try {
                    ...
                    \Log::error('Login email verification OTP mail failed: ' . $e->getMessage(), ['email' => $account->email ?? null]);
                    return back()->withInput()->withErrors([
            ...
            // 2. Email verified → sendLoginResponse() calls authenticated() (login_time + employer notify once)
    ...
    /**
     * After password login: session markers + employer login notification (OTP login calls this too).
     */
    protected function onAccountLoggedIn(Request $request, Account $account): void
    {
    ...
    /**
     * The user has been authenticated (password flow via sendLoginResponse).
     */
    protected function authenticated(Request $request, $account)
    {
        if ($account instanceof Account) {
            $this->onAccountLoggedIn($request, $account);
        }
    ...
    public function sendEmailOtp(Request $request)
    {
        ...
        $prevOtp = $request->session()->get('login_otp');
        $preservePendingSignup = is_array($prevOtp)
            && ! empty($prevOtp['pending_signup'])
            && isset($prevOtp['email'])
            && mb_strtolower((string) $prevOtp['email']) === mb_strtolower($emailInput);

        $account = $this->findAccountByEmailInsensitive($emailInput);

        $pendingSignup = false;
        if ($account) {
            $pendingSignup = false;
        } elseif ($preservePendingSignup) {
            $pendingSignup = true;
        } elseif (
            $request->boolean('from_job_apply')
            && JobBoardHelper::isRegisterEnabled()
            && $this->isJobDetailApplyRedirect($request->input('redirect'))
        ) {
            $pendingSignup = true;
        } else {
        ...
        $sessionEmail = $account ? (string) $account->email : mb_strtolower($emailInput);
        ...
        if ($pendingSignup) {
            $loginOtpPayload['pending_signup'] = true;
        }

        $request->session()->put('login_otp', $loginOtpPayload);

        $this->storePostLoginRedirectFromRequest($request);

        // Send OTP via email
        try {
            Mail::send([], [], function ($message) use ($sessionEmail, $otp, $account) {
                $helloP = $account && trim((string) $account->first_name) !== ''
                    ? '<p>Hello ' . e($account->first_name) . ',</p>'
                    : '<p>Hello ' . e(__('there')) . ',</p>';
            ...
            \Log::error('Login sendEmailOtp mail failed: ' . $e->getMessage(), ['email' => $sessionEmail ?? null]);
            return response()->json([
                'error' => true,
    ...
    public function sendWhatsAppOtp(Request $request)
    {
        ...
        try {
            ...
            \Log::error('Login sendWhatsAppOtp failed: ' . $e->getMessage());
            return response()->json([
                'error' => true,
    ...
    public function verifyOtpLogin(Request $request)
    {
        try {
            ...
            // Find account (or create job seeker when job-detail apply flow sent pending_signup)
            $account = null;
            if ($otpData['type'] === 'email') {
                ...
                if (! empty($otpData['pending_signup'])) {
                    $account = $this->findAccountByEmailInsensitive($otpData['email']);
                    if (! $account) {
                        if (! JobBoardHelper::isRegisterEnabled()) {
                            ...
                            return response()->json([
                                'error' => true,
                                'message' => __('Registration is currently disabled.'),
                            ]);
                        }
                        try {
                            $account = $this->createJobSeekerAccountFromJobApplyOtp($otpData['email']);
            ...
            $nextUrl = $this->resolvePostLoginRedirectAfterOtp($request, $account);
    ...
    /**
     * Remember where to redirect after OTP login (e.g. back to job detail to apply).
     */
    protected function storePostLoginRedirectFromRequest(Request $request): void
    {
        ...
        if ($this->isSafePostLoginRedirect($url)) {
            $request->session()->put('post_login_redirect', $url);
        }
    }

    protected function isSafePostLoginRedirect(string $url): bool
    {
        $url = trim($url);
        if ($url === '') {
            return false;
        ...
        return $host && $appHost && strcasecmp((string) $host, (string) $appHost) === 0;
    }

    protected function resolvePostLoginRedirectAfterOtp(Request $request, Account $account): string
    {
        $stored = $request->session()->pull('post_login_redirect');
        if (is_string($stored) && $stored !== '' && $this->isSafePostLoginRedirect($stored)) {
            return str_starts_with($stored, '/') && ! str_starts_with($stored, '//')
                ? url($stored)
                : $stored;
        }

        return $this->getDashboardUrlForAccount($account);
    }

    protected function findAccountByEmailInsensitive(string $email): ?Account
    {
        $email = trim($email);
        if ($email === '') {
            return null;
    ...
    /**
     * Only URLs that look like a public job detail page (used with from_job_apply OTP signup).
     */
    protected function isJobDetailApplyRedirect(mixed $url): bool
    {
        ...
        return str_contains(strtolower($path), '/jobs/');
    }

    protected function createJobSeekerAccountFromJobApplyOtp(string $email): Account
    {
        $email = mb_strtolower(trim($email));
        $localPart = explode('@', $email)[0] ?? 'user';
        $displayName = Str::title(str_replace(['.', '_', '-'], ' ', $localPart));

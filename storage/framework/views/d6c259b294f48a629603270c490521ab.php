...
<?php $__env->startSection('content'); ?>
    <?php if(request()->route() && request()->route()->getName() === 'public.account.companies.index' && \Botble\JobBoard\Facades\JobBoardHelper::isEnabledCreditsSystem()): ?>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="additionalInstitutionDeductModalLabel"><?php echo e(__('Add institution')); ?></h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p class="mb-0"><?php echo e(__(':credits credits will be deducted first. Then the Add institution form will open. Continue?', ['credits' => $additionalProfileCredits])); ?></p>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"><?php echo e(__('Cancel')); ?></button>
                            <button type="button" class="btn btn-primary" id="additionalInstitutionDeductConfirmBtn"><?php echo e(__('Continue')); ?></button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <script>
        (function() {
            var companyCount = <?php echo e($companyCount); ?>;
            var institutionLocked = <?php echo e($institutionLocked ? 'true' : 'false'); ?>;
            var walletUrl = <?php echo e(json_encode(route('public.account.wallet'))); ?>;
            var createUrl = <?php echo e(json_encode(route('public.account.companies.create'))); ?>;
            var deductUrl = <?php echo e(json_encode(route('public.account.additional-employer-profile.deduct'))); ?>;
            var insufficientMsg = <?php echo e(json_encode(__('Insufficient credits. You need :credits credits to add another institution. Redirecting to Wallet.', ['credits' => $additionalProfileCredits]))); ?>;

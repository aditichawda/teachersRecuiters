    ...
    <div class="jp-card jp-card-collapsible" data-section="5">
        ...
        <div class="jp-group">
            ...
            <div class="jp-option-cards">
                ...
                <label class="jp-option-card <?php echo e($appLocType === 'anywhere' ? 'selected' : ''); ?>" onclick="selectOption(this, 'application_location_type')">
                    <input type="radio" name="application_location_type" value="anywhere" <?php echo e($appLocType === 'anywhere' ? 'checked' : ''); ?>>
...
<div id="buy-credits-popup" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:9999; align-items:center; justify-content:center;">
    <div style="background:#fff; border-radius:12px; padding:24px; max-width:400px; margin:20px; box-shadow:0 4px 20px rgba(0,0,0,0.2);">
        ...
        ?>
        <?php if($creditsEnabledPopup): ?>
            <div class="border rounded p-3 mb-3" style="background:#f8fafc;">
                <div class="mb-2" style="font-weight:600;"><?php echo e(__('Recharge Wallet')); ?></div>
                <div class="d-flex gap-2 align-items-center">
                    <input
                        ...
                        placeholder="<?php echo e(__('Enter amount (min ₹100)')); ?>"
                        style="max-width: 180px;"
                        <?php if(! $canRechargeWallet): echo 'disabled'; endif; ?>
                    >
                    <form method="post" action="<?php echo e(route('public.account.wallet.recharge.start')); ?>" id="wallet-recharge-form" style="margin:0;">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="amount_inr" id="wallet-recharge-amount-hidden" value="">
                        <button type="submit" class="btn btn-success" id="wallet-recharge-btn" <?php if(! $canRechargeWallet): echo 'disabled'; endif; ?>>
                ...
                <div class="small mt-2" id="wallet-recharge-hint" style="color:#6b7280;">
                    <?php if(! $canPost): ?>
                        <?php echo e(__('Wallet recharge is available only with an active hiring plan.')); ?>

                    <?php elseif(((int) ($accountCredits ?? 0)) > 0): ?>
                        <?php echo e(__('Recharge is available when your wallet credits are exhausted.')); ?>

                    <?php else: ?>
                        <?php echo e(__('You will be redirected to Razorpay to complete the payment.')); ?>

                    <?php endif; ?>
                </div>
                <div class="small mt-1" style="color:#6b7280;">
                    <?php echo e(__('Wallet credits have unlimited validity and can be used with any active hiring plan.')); ?>
...
var canRechargeWallet = <?php echo e(($creditsEnabledPopup ?? false) && ($canPost ?? false) && ((int) ($accountCredits ?? 0) <= 0) ? 'true' : 'false'); ?>;
...
if (rechargeForm && rechargeAmount && rechargeHidden) {
    rechargeForm.addEventListener('submit', function(e) {
        ...
        if (!val || val < 100) {
            e.preventDefault();
            if (rechargeError) {
                rechargeError.textContent = '<?php echo e(__("Minimum recharge amount is ₹100.")); ?>';

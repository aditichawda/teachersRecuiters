                        ...
                        <div class="row align-items-end">
                            ...
                            <div class="col-md-3 mb-2">
                                ...
                                <div class="jp-suggest-wrap">
                                    <input type="text" class="form-control form-control-sm work-city-search" value="<?php echo e($loc['city_name'] ?? ''); ?>" placeholder="<?php echo e(__('Search city...')); ?>" autocomplete="off" data-index="<?php echo e($index); ?>">
                                    <div class="jp-suggest-list work-city-suggestions" data-index="<?php echo e($index); ?>"></div>
                                </div>
                            </div>
                            <div class="col-md-3 mb-2">
                                <label class="form-label small"><?php echo e(__('State')); ?></label>
                                <input type="text" class="form-control form-control-sm work-state-display" value="<?php echo e($loc['state_name'] ?? ''); ?>" readonly style="background:#f5f5f5;">
                            </div>
                            <div class="col-md-3 mb-2">
                                <label class="form-label small"><?php echo e(__('Country')); ?></label>
                                <input type="text" class="form-control form-control-sm work-country-display" value="<?php echo e($loc['country_name'] ?? ''); ?>" readonly style="background:#f5f5f5;">
                            </div>
                            <?php else: ?>
                            <div class="col-md-3 mb-2">
                                <label class="form-label small"><?php echo e(__('Country')); ?></label>
    ...
    <div class="profile-section">
        ...
        <div class="profile-section-body">
            ...
            <div class="row">
                ...
                <div class="col-12 mb-3">
                    <label class="form-label"><?php echo e(__('Key Skills & Competencies')); ?><span class="field-help-icon" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-trigger="hover focus click" title="<?php echo e(__('Add your core teaching or professional skills relevant to your role. Stored with your profile and used for job matching.')); ?>"><i class="fa fa-question-circle"></i></span></label>
                    <?php
                        $jobSkillsList = $jobSkills ?? collect();
                        $tagifyList = $jobSkillsList->map(fn($s) => ['id' => (string) $s->id, 'name' => $s->name ?? ''])->values()->toArray();
                    ?>
                    <input type="text" class="tags form-control list-tagify" style="padding:0px;" id="favorite_skills" name="favorite_skills" value="<?php echo e(implode(',', array_map('strval', $selectedJobSkills ?? []))); ?>" data-keep-invalid-tags="false" data-list="<?php echo e(json_encode($tagifyList)); ?>" data-user-input="false" placeholder="<?php echo e(count($tagifyList) ? __('Select from the list') : __('No skills in list yet; admin can add skills in Job Skills.')); ?>"/>
                    <?php if(count($tagifyList) == 0): ?>
                        <p class="form-text small text-muted mt-1"><?php echo e(__('Skills list is managed by admin. You can add your skills once the list is available.')); ?></p>
                    <?php endif; ?>
                </div>
                <?php endif; ?>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    (function() {
        ...
        function loadCities(k) {
            ...
            var url = '<?php echo e(route("ajax.search-cities")); ?>' + (k && k.length >= 2 ? '?k=' + encodeURIComponent(k) : '?default_country=1&page=1');
    ...
    (function() {
        ...
        searchEl.addEventListener('input', function() {
            ...
            searchTimeout = setTimeout(function() {
                ...
                suggestionsEl.style.display = 'block';
                fetch('<?php echo e(route("ajax.search-cities")); ?>?k=' + encodeURIComponent(k), { headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } })
                    .then(function(r) {
                        if (!r.ok) throw new Error('HTTP ' + r.status);
                        var ct = r.headers.get('Content-Type') || '';
                        if (!ct.includes('application/json')) throw new Error('Invalid response');
        ...
        if (countrySel && countrySel.value && stateSel) {
            fetch('<?php echo e(route("ajax.states-by-country")); ?>?country_id=' + countrySel.value, { headers: ajaxHeaders })
                ...
                .then(function(data) {
                    ...
                    var opts = [{ value: '', text: '<?php echo e(__("Select")); ?>' }];
                    ...
                    if (stateId && stateSel.value && citySel) {
                        fetch('<?php echo e(route("ajax.cities-by-state")); ?>?state_id=' + stateSel.value, { headers: ajaxHeaders })
                            ...
                            .then(function(data2) {
                                ...
                                var opts2 = [{ value: '', text: '<?php echo e(__("Select")); ?>' }];
    ...
    if (workLocContainer) {
        workLocContainer.addEventListener('change', function(e) {
            if (e.target.classList.contains('work-pref-country')) {
                ...
                if (stateSel) window.TR.setNativeAndTomSelectOptions(stateSel, [{ value: '', text: '<?php echo e(__("Select")); ?>' }], '');
                if (citySel) window.TR.setNativeAndTomSelectOptions(citySel, [{ value: '', text: '<?php echo e(__("Select")); ?>' }], '');
        ...
        function initRow(row) {
            ...
            function renderItems(cities) {
                if (!Array.isArray(cities) || cities.length === 0) {
                    list.innerHTML = '<div class="jp-suggest-item" style="cursor:default;color:#6b7280;"><?php echo e(__("No cities found")); ?></div>';
                    openDropdown();
                    return;
                }
                var html = '';
            ...
            function fetchCities(keyword) {
                ...
                var url = '<?php echo e(route("ajax.search-cities")); ?>' + (k.length >= 2 ? ('?k=' + encodeURIComponent(k)) : '?default_country=1&page=1');
                list.innerHTML = '<div class="jp-suggest-item" style="cursor:default;color:#6b7280;">' + (k.length >= 2 ? '<?php echo e(__("Searching...")); ?>' : '<?php echo e(__("Loading...")); ?>') + '</div>';
                openDropdown();
                fetch(url, { headers: ajaxHeaders })
                    ...
                    .catch(function() {
                        list.innerHTML = '<div class="jp-suggest-item" style="cursor:default;color:#6b7280;"><?php echo e(__("Error loading cities")); ?></div>';
                        openDropdown();
                    });
            }
...
function addWorkLocation() {
    ...
    if (useLocationDropdowns) {
        ...
        html = '<div class="removable-item work-location-item">' +
            ...
            '<div class="col-md-3 mb-2"><label class="form-label small"><?php echo e(__("City")); ?></label>' +
            '<div class="jp-suggest-wrap"><input type="text" class="form-control form-control-sm work-city-search" placeholder="<?php echo e(__("Search city...")); ?>" autocomplete="off" data-index="' + idx + '">' +

@php
    ...
    if ($withCategories) {
        $jobFeaturedCategories = $jobFeaturedCategories->where('jobs_count')->sortByDesc('jobs_count');
    }
@endphp

@if ($withCategories && $jobFeaturedCategories->isNotEmpty())
    <div class="widget-popular-title mt-4">
        <h2 class="h6">{{ __('Featured') }}</h2>
        <ul class="list-inline">
            @foreach ($jobFeaturedCategories as $item)
